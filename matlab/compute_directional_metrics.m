...
%
%  PdB: power spectral density of pressure autospectrum in dB 
...
if ~use_wavelet
    PdB=4+10*log10(2*pressure_autospectrum./(Nfft*Fs));  %%Power spectral density output in dB
else
...
for J=1:length(metric_type)  %%for each request
    ...
    switch metric_type{J}
        ...
        case lower('Polarization')
            ...
            %sqrt(s1^2+s2^2+s3^2)=sqrt(s1^2+4*|Vxy|^2
