...
Next, each transient detection was compared against the manual database to see whether the transient's time window overlapped a manual annotation time window by 50\% or more, ignoring annotations flagged as seals or unknown biologics.  If this overlap threshold was exceeded, the transient was flagged as an 'annotation' and assigned the classification label associated with the annotation.  If a transient was sufficiently long to encompass multiple annotations, the earliest annotation was used.  **Rerunning the CFAR detector and the manual/automated overlap-matching step across every date/site/DASAR combination in Table~\ref{tab:training_dates} (254,963 manual whale-call annotations in total) found that 5,302 (2.08\%) did not overlap any automated transient detection.**
